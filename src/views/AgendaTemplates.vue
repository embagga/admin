  ...
  <CRow>
    <CCol :xs="3">
      <CCard class="mb-4">
        <CCardHeader class="flex justify-between p-2">
          <span class="mb-1"> <strong>Select Event Types</strong> </span>
        </CCardHeader>
        <CCardBody>
          <CTable small>
            <CTableHead>
    ...
    <CCol :xs="9">
      <CCard class="mb-4">
        <CCardHeader class="flex justify-between">
          <span> Configure <strong>Schedule Template</strong> </span>
          <CButton
            color="primary"
            class="float-right"
            size="sm"
          ...
          <CTable small>
            <CTableHead>
              <CTableRow>
                ...
                <CTableHeaderCell scope="col">Venue Kind</CTableHeaderCell>
            ...
            <CTableBody>
              <CTableRow v-for="(row, index) in templateData" :key="index">
                ...
                <CTableDataCell>{{ groups[row.is_group.toString()] }}</CTableDataCell>
                <CTableDataCell>{{ row.title }}</CTableDataCell>
                <CTableDataCell>{{ row.start_date }}</CTableDataCell>
    ...
    <CModal :visible="showModal" @close="handleModalClose">
      <CModalHeader dismiss @close="handleModalClose" class="flex">
        <CModalTitle>
          {{ mode === 'add' ? 'Add' : 'Edit' }} Schedule Agenda Template Items
        </CModalTitle>
      </CModalHeader>
      <CModalBody>
        <CForm class="text-center">
          <CInputGroup class="mb-3">
            <CInputGroupText> Venue: </CInputGroupText>
            <CFormSelect v-model="form.is_group" id="inputGroupVenueInputs">
              <option value="1">Primary Venue</option>
              <option value="2">Other Venue</option>
            </CFormSelect>
          </CInputGroup>
          ...
          <CInputGroup class="mb-4">
            ...
            <CFormInput type="time" v-model="form.start_date" />
          </CInputGroup>
        </CForm>
      </CModalBody>
      <CModalFooter>
...
export default {
  ...
  methods: {
    ...
    loadTemplateItems() {
      ...
      Bee.get(data, false, vm)
        .then((res) => {
          vm.templateData = res.agenda_templates
        })
        .catch((errors) => {
          console.log('Errors', errors)
        })
    ...
    handleClickEdit(row) {
      ...
      this.form.is_group = row.is_group
      this.mode = 'edit'
      this.showModal = true
    },
    ...
    onHandleSubmit() {
      ...
      var data = {
        agenda_template: {
          ...
          is_group: thisVM.form.is_group,
          title: thisVM.form.title,
          start_date: thisVM.form.start_date,
          start_date_time: 0,
          end_date: '',
          end_date_time: -1,
          duration: 0,
      ...
      if (thisVM.mode === 'edit') {
        data.agenda_template._w = [['id', 'e', thisVM.form.id]]
        method = 'update'
      }
      //add token
      Bee.token = localStorage.getItem('token')
